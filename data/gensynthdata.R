...
generate_reward_state <- function(input) {
  ...
  if(pred.data$decision.time > 5) {
    ...
    next.decision.time = (pred.data$decision.time + 1)%% 5
    if (next.decision.time == 1) {
      next.day = pred.data$day + 1 
    } else {
      next.day = pred.data$day
    }
  }
  
  current.row = which(analysis.data$id == pred.data$id & analysis.data$day == next.day & analysis.data$decision.time == next.decision.time)
